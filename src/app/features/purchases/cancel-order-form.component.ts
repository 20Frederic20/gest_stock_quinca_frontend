...
@Component({
  selector: 'app-cancel-order-form',
  imports: [ReactiveFormsModule, FieldErrorComponent],
  templateUrl: './cancel-order-form.component.html',
  styleUrl: './cancel-order-form.component.css',
})
export class CancelOrderFormComponent {
  private fb = inject(FormBuilder);
  private service = inject(PurchaseOrdersService);

  orderId = input.required<string>();
  /** e.g. "CDE-COT-2026-00042 — Ciments du Bénin". */
  summary = input('');
  ...
  // Same rules as CancelOrderRequest on the backend.
  ...
  submit(): void {
    ...
    this.service.cancel(this.orderId(), this.form.getRawValue().reason.trim()).subscribe({
      next: order => {
