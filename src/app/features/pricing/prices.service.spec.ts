...
// No /v1: the backend ArticlePriceController is mapped on /api.
...
describe('PricesService', () => {
  ...
  it('gets the history of a packaging for one privilege', () => {
    ...
    const req = httpTesting.expectOne(r => r.url === '/api/packagings/k1/prices/history');
    expect(req.request.method).toBe('GET');
  ...
  it('gets the applicable price, today unless a date is given', () => {
    ...
    const today = httpTesting.expectOne(r => r.url === '/api/packagings/k1/prices/applicable');
    ...
    const dated = httpTesting.expectOne(r => r.url === '/api/packagings/k1/prices/applicable');
